...
public class MemIndex implements HeapSize{
  ...
  void loadIndex()throws IOException{
  	...
  	LOG.info("loadIdx: path: " + store.indexPath);
  	...
  	do{
        ...
        //LOG.info("loadIdx: put: " + key + " ;;; value: " + value);
  		dataSize.addAndGet(value.getSize());
  	}while(true);
  	LOG.info("finish load index, duration :" + (System.currentTimeMillis() - startTime) + "(ms), path: " + store.indexPath);
  }
  ...
  void dump() throws IOException{
    ...
    LOG.info("finish dump index, duration :" + (System.currentTimeMillis() - startTime) + "(ms), tmpPath: " + tmpPath);
    startTime = System.currentTimeMillis();
    
    //rename the file
    if(fs.exists(store.indexPath)) fs.delete(store.indexPath, true);
    LOG.info("move " + tmpPath + " to " + store.indexPath);
    fs.rename(tmpPath, store.indexPath);

    LOG.info("finish move index, duration :" + (System.currentTimeMillis() - startTime) + "(ms), path: " + store.indexPath);
  }
