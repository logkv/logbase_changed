...
public class HLogKey implements WritableComparable<HLogKey> {
  ...
  /**
   * This function is only used by log-only store
   * For each entry in HLog, no region and table info is required
   * As a result, the region name and the table name is byte[0].
   * the length of these two empty string is 1 in respect
   * @return
   */
  public int getLengthWithEmptyRegion(){
  	// length(regionName) + length(tablename) + long + long + byte
  	return 1 + 1 + 8 + 8 + 1;
  }
  
  public int getSerializationLength(){
  	int len = getLengthWithEmptyRegion();
