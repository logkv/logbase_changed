...
import org.apache.hadoop.hbase.regionserver.wal.LogBaseIndexConstructScanner;
...
import sg.edu.nus.logbase.crindex.LogIndexCRIndexSingleDimensionFullScan;
import sg.edu.nus.logbase.crindex.LogIndexConfigurationSet;
...
public class Store implements HeapSize {
  ...
  //protected final MemIndex blockIndex;
  ...
  /**
   * WangSheng 2013-4-3: A new generalized interface replaces the LogStoreIndex
   */
  //public LogIndexInterface logIndexUltimate;
  //2013-12-24 huanghao
  public Map<byte[], LogIndexInterface> logIndexUltimateMap = new ConcurrentHashMap<byte[], LogIndexInterface>();
  ...
  throws IOException {
    ...
    //this.blockIndex = new MemIndex(this);
    ...
    //2013-12-13 wangsheng -----------move it to a new function
    // do not know what interface to give for creating index, we manually create it first

    //12-13, to delete index
    // logIndexUltimate = null;
    
  }
  

  private LogIndexInterface constructIndex(byte[] indexedColumn, int cellNum, int blockSize) throws IOException {
	  LogIndexInterface logIndexUltimate =null;
	    LogIndexConfigurationSet config = new LogIndexConfigurationSet();
	    config.setConfiguration("cell_per_tuple", String.valueOf(cellNum));
		config.setConfiguration("tuple_per_block", String.valueOf(blockSize));	
	    logIndexUltimate = new LogIndexCRIndexSingleDimensionFullScan(indexedColumn, this, config);
	    
	    // 2013-12-22 wangsheng need to revise the function below to re-scan the column and build index
	    // 2013-12-30 wangsheng
	    LogBaseIndexConstructScanner scanner = new LogBaseIndexConstructScanner(this.currentLog);
	    KeyValue kv = new KeyValue();
	    LogEntryOffset offset = new LogEntryOffset();
	    ...
	    while (scanner.next(kv, offset) == true)
	    {
	    	logIndexUltimate.add(kvs, offsets);
	    }
	    
	  return logIndexUltimate;
  }
  ...
  protected long add(final KeyValue kv) {
    ...
    try {
        ...
        //System.out.println("Store.add: In Single Function");
  ...
  protected void add(List<KeyValue> kvs) throws IOException {
    ...
    //2013-12-13 wangsheng
    //System.out.println("Store.add: In Batch Function");
    //System.out.println("Store.add: key = "+Bytes.toString(kvs.get(0).getRow()));
    //System.out.println("Store.add: column = "+Bytes.toString(kvs.get(0).getQualifier()));
    ...
    try {
    	...
    	// wangsheng need to help indicate which column of which table is adding new kv
    	// update cr-index
    	// 2013-12-30 wang sheng
    	for(int i = 0;i < kvs.size();i ++){
    		KeyValue kv = kvs.get(i);
    		LogIndexInterface logIndexUltimate = logIndexUltimateMap.get(kv.getQualifier());
    		if (logIndexUltimate == null) continue;
    		logIndexUltimate.add(kvs, offsets);
    	}
...
//    	LOG.info("getScanner: " + scan);
    	
    	if (scan.toDeleteCRIndex){
    		//String table = new String(scan.tableName);
    		//String indexedColumn = new String(scan.idxColumn);
    		//String idxID = table+"_"+indexedColumn;
    		logIndexUltimateMap.put(scan.idxColumn, logIndexUltimate);
    	}else if (scan.toCreateCRIndex){
    		//String table = new String(scan.tableName);
    		//String indexedColumn = new String(scan.idxColumn);
    		//String idxID = table+"_"+indexedColumn;
    		logIndexUltimate = constructIndex(scan.idxColumn, scan.cellNum, scan.blockSize);
    		logIndexUltimateMap.put(scan.idxColumn, logIndexUltimate);
    	}else if (scan.CRindex){
    		//String table = new String(scan.tableName);
    		//String indexedColumn = new String(scan.idxColumn);
    		//String idxID = table+"_"+indexedColumn;
    		logIndexUltimate = logIndexUltimateMap.get(scan.idxColumn);
    	}

    	getLog().rollWriter();

    	return  new LogBaseScanner(this.family.getName(), scan, this.currentLog, this.memIndex, this.logCache, logIndexUltimate);
    	// (pre i am here 5.6) i am here 6
