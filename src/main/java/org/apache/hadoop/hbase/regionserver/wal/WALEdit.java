...
public class WALEdit implements Writable, HeapSize {
  ...
  // TODO: turn it into private
  private int prefixLength(){
    return Bytes.SIZEOF_INT * 2;
  }

  List<LogEntryOffset> calculateOffset(int filenum, long beginOffset, List<LogEntryOffset> offsets){
  	if(offsets == null) offsets = new ArrayList<LogEntryOffset>();
  	offsets.clear();
  	beginOffset += this.prefixLength();
    for(KeyValue kv : kvs){
    	// TODO: here the creation maybe very slow when scan the files
      LogEntryOffset offset = new LogEntryOffset(filenum, (int)kv.getSerializationLength(), (int)beginOffset);
      beginOffset += kv.getSerializationLength();
      offsets.add(offset);
    }
    return offsets;
  }
