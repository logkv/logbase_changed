...
public abstract class ZooKeeperNodeTracker extends ZooKeeperListener {
  ...
  throws InterruptedException {
    ...
    boolean bln_tmp = false;
    //==============================================
    String tmp = this.node;
    if (this.data ==null)
    {
    	this.watcher.registerListener(this);
    	try {
    		  bln_tmp =ZKUtil.watchAndCheckExists(watcher, node);
    	      if(ZKUtil.watchAndCheckExists(watcher, node)) {
    	        byte [] data = ZKUtil.getDataAndWatch(watcher, node);
    	        if(data != null) {
    	          //this.data = data;
  ...
  public synchronized byte [] blockUntilAvailable_2(long timeout)
  throws InterruptedException {
    if (timeout < 0) throw new IllegalArgumentException();
    boolean notimeout = timeout == 0;
    long startTime = System.currentTimeMillis();
   ...
   */
  public synchronized byte [] getData() {//public synchronized byte [] getData() {
    return data;
  }
