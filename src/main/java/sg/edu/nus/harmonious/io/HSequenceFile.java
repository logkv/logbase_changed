...
//  public static final int SYNC_INTERVAL = 100*SYNC_SIZE; 
  ...
  public static class Writer implements java.io.Closeable {
    ...
    public Writer(FileSystem fs, Configuration conf, Path name,
      ...
      LOG.info("createWriter, replication: " + replication + " name: " + name);
...
//    }
  ...
  public static class Reader implements java.io.Closeable {
    ...
    //huanghao
    public Reader(FileSystem fs, Path file, Configuration conf, long offset)
    	      throws IOException {
    	      this(fs, file, conf.getInt("io.file.buffer.size", 4096), conf, false, offset);
    	    }
    ...
    //huanghao
    private Reader(FileSystem fs, Path file, int bufferSize,
            Configuration conf, boolean tempReader, long offset) throws IOException {
this(fs, file, null, bufferSize, offset, fs.getFileStatus(file).getLen(),
   conf, tempReader);
}
    ...
    private synchronized void seekToCurrentValue() throws IOException {
      if (!blockCompressed) {
        if (decompress) {
        	//System.out.println("***Perform decompress***");//never be here
    ...
    public synchronized void getCurrentValue(Writable val) 
      ...
      //System.out.println("--[[before Position, val= "+val);
      ...
      if (!blockCompressed) {
        ...
        //System.out.println("--[[after Position, val= "+val);
    ...
    public synchronized boolean next(Writable key) throws IOException {
      ...
      if (!blockCompressed) {
    	  //System.out.println(" [[ no blockCompressed ]]");
    	  
    	  //System.out.println(" - [[ before reset, outBuf="+outBuf.toString());
        outBuf.reset();
          //System.out.println(" - [[ after reset, outBuf="+outBuf.toString());
        
        //read next key/value pair into outBuf,returns the length of the key read, or -1 if at end of file
        ...
        //System.out.println("- [[ valBuffer = "+ (valBuffer.getData()));
    ...
    public synchronized boolean next(Writable key, long SeqLogNum, int offset) throws IOException {
        if (key.getClass() != getKeyClass())
          throw new IOException("wrong key class: "+key.getClass().getName()+" is not "+keyClass);

        if (!blockCompressed) {
//        	in.seek(offset);
//        	int lenTmp = in.readInt();
//        	byte[] bTmp = new byte[lenTmp];
//        	in.readFully(bTmp, 0, lenTmp);
    ...
    public synchronized boolean skip_to_target_key(HLogKey key, WALEdit val, long seqNum, int offset)
    	      throws IOException {

    	      boolean more = next(key,  seqNum,  offset);
    	      while(more){
    	    	  HLogKey tmp = (HLogKey)key;
    	    	  if (tmp.getLogSeqNum()==seqNum){
    	    		  break;
    	    	  }else {
    	    		  more = next(key);
    	    	  }
    ...
    private synchronized int readRecordLength() throws IOException {
      ...
      if (version > 1 && sync != null &&
        ...
        //System.out.println("--[[ after readFully, in.getPos()="+in.getPos());
        length = in.readInt();                  // re-read length
        //System.out.println("--[[ after readFully, in.readInt()="+length);
      } else {
    	  //System.out.println("--[[hehrefdadfadf]]");
        syncSeen = false;
      }
    ...
    public synchronized int next(DataOutputBuffer buffer) throws IOException {
      ...
      try {
    	   ...
    	   //System.out.println("--[[ 1.1, in.pos = "+in.getPos());
        int length = readRecordLength();
           //System.out.println("--[[ 1.2, in.pos = "+in.getPos());
        
        if (length == -1) {
          return -1;
        }
           //System.out.println("--[[ 2.1, in.pos = "+in.getPos());
        int keyLength = in.readInt();
           //System.out.println("--[[ 2.2, in.pos = "+in.getPos());
